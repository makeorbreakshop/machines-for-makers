...
interface MachineMatcherProps {
  ...
  onMatchingComplete: (matches: Map<string, string>, selectedCsvRows?: any[]) => void;
  onBack: () => void;
  uploadedFile?: File;
...
const FastTableRow = memo(function FastTableRow({
  product,
  productIndex,
  result,
  currentMatch,
  ...
  const handleClick = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    onCheckboxClick(productIndex, e.shiftKey);
  }, [productIndex, onCheckboxClick]);

  const machine = useMemo(() => {
    if (!topSuggestion) return null;
    return availableMachines.find(m => m.id === topSuggestion.machine_id);
  }, [topSuggestion, availableMachines]);

  const confidence = topSuggestion?.confidence_score || 0;
  ...
  return (
    <TableRow className={isSelected ? 'bg-blue-50' : ''}>
      ...
      <TableCell>
        {topSuggestion && confidenceBadge}
      </TableCell>
      <TableCell>
        {machine ? (
          <span className="font-medium">
            ${machine.Price ? parseFloat(machine.Price).toLocaleString() : 'N/A'}
          </span>
        ) : (
  ...
  return (
    ...
    prevProps.currentMatch === nextProps.currentMatch &&
...
export function MachineMatchingInterface({ 
    ...
    if (isShift && lastSelectedIndex >= 0) {
      ...
      for (let i = start; i <= end; i++) {
        if (visibleProducts[i]) {
          newSelected.add(visibleProducts[i]);
        }
      }
  ...
  }, [visibleProducts, lastSelectedIndex]);
  ...
  const handleBatchImport = async () => {
    // Get selected CSV rows
    const selectedCsvRows = csvData.filter(row => {
      const product = row.Products || row.products || '';
      return selectedRows.has(product);
    });
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex justify-between items-center gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button onClick={() => {
              ...
              const remainingCsvRows = csvData.filter(row => {
                const product = row.Products || row.products || '';
                return !importedProducts.has(product);
              });
              onMatchingComplete(finalMatches, remainingCsvRows);
      ...
      {/* Test Preview Modal removed - all imports now process full dataset */}
      {false && (
