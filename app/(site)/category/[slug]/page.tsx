...
export default async function CategoryPage({ params }: { params: { slug: string } }) {
  ...
  const breadcrumbItems = [
    { label: "Laser Cutters", href: "/category/laser-cutters" },
  ...
  const categorySchema = {
    ...
    url: `https://machinesformakers.com/category/${categorySlug}`,
    mainEntity: {
      "@type": "ItemList",
...
function getCategoryName(slug: string): string {
  const categoryMap: Record<string, string> = {
    ...
    "laser-cutters": "Laser Cutters & Engravers",
  }
...
function getCategoryDescription(slug: string): string {
  const descriptionMap: Record<string, string> = {
    ...
    portable:
      "Portable lasers are compact, lightweight engraving systems designed for on-the-go use. These handheld or small desktop units sacrifice some power and work area for exceptional portability. They're perfect for crafters, small business owners who attend events, or anyone with limited workspace.",
    "laser-cutters":
      "Laser cutters and engravers use focused light beams to cut, mark, or engrave materials with precision. These versatile machines come in various types, sizes, and power levels to suit different applications. From hobbyist models to industrial systems, laser cutters have revolutionized manufacturing, crafting, and prototyping across numerous industries.",
  }
...
function getCategoryFAQ(slug: string, question: string): string {
  const faqMap: Record<string, Record<string, string>> = {
    ...
    portable: {
      what: "A portable laser is a compact, lightweight laser engraving device designed for mobility and ease of use. These can be handheld units or small desktop devices that can be easily transported and set up in different locations.",
      uses: "Portable lasers are primarily used for engraving small items like jewelry, electronics, crafts, and personalized gifts. They're ideal for crafters who attend events, small business owners with limited space, or hobbyists who need flexibility.",
      cost: "Portable lasers typically range from $200 to $1,000. Basic handheld units start around $200-400, while more capable portable desktop units can cost $500-1,000.",
    },
    "laser-cutters": {
      what: "Laser cutters and engravers are machines that use focused laser beams to cut through or mark materials with high precision. They come in various types including CO2, fiber, and diode lasers, each suited to different applications and materials.",
      uses: "Laser cutters and engravers are incredibly versatile tools used across numerous industries and hobbies. They can cut and engrave materials like wood, acrylic, leather, fabric, paper, and some metals, creating everything from signage and artwork to product components and prototypes.",
      cost: "Laser cutters and engravers vary widely in price based on type, power, and features. Entry-level diode lasers start around $200-500, mid-range desktop CO2 lasers cost $2,000-5,000, and professional/industrial systems can range from $10,000 to $50,000+.",
    },
  }
