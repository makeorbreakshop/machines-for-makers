...
const convertKitApiSecret = process.env.CONVERTKIT_API_SECRET; // Using API Secret, not API Key
...
console.log('- ConvertKit API Secret:', convertKitApiSecret ? `✓ Set (${convertKitApiSecret.substring(0, 10)}...)` : '✗ Missing');
  ...
  while (hasMore) {
    try {
      ...
      const response = await fetch(
        `https://api.convertkit.com/v3/subscribers?api_secret=${convertKitApiSecret}&page=${page}`,
        {
          method: 'GET',
          headers: {
...
async function getSubscriberTags(subscriberId) {
  try {
    const response = await fetch(
      `https://api.convertkit.com/v3/subscribers/${subscriberId}?api_secret=${convertKitApiSecret}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
...
async function importSubscribers() {
  ...
  for (let i = 0; i < allSubscribers.length; i++) {
    const subscriber = allSubscribers[i];
    console.log(`Processing ${i + 1}/${allSubscribers.length}: ${subscriber.email_address}`);
    ...
    // Import all subscribers, even if we don't recognize their tags
    if (tagNames.length === 0) {
      // If no tags, try to guess based on other factors or mark as general
      formName = 'General';
      source = 'unknown';
    }
    ...
    if (error) {
      ...
      console.log(`✓ Imported ${subscriber.email_address} (${formName}) - Tags: ${tagNames.join(', ') || 'none'}`);
      totalImported++;
    }
