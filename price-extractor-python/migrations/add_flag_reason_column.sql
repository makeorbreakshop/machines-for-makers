-- Migration to add flag_reason column and fix price_history previous_price, price_change and percentage_change
-- Also adds a function and trigger to automatically calculate these values if not provided

-- Add flag_reason column to machines_latest if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_name = 'machines_latest' AND column_name = 'flag_reason'
  ) THEN
    ALTER TABLE machines_latest ADD COLUMN flag_reason TEXT;
    RAISE NOTICE 'Added flag_reason column to machines_latest table';
  ELSE
    RAISE NOTICE 'flag_reason column already exists in machines_latest table';
  END IF;
END $$;
...
-- Update existing records with manual_review_flag=true to have a default reason
UPDATE machines_latest 
SET flag_reason = 'Significant price change (added by migration)'
WHERE manual_review_flag = true AND (flag_reason IS NULL OR flag_reason = ''); 